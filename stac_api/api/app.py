...
class StacApi:
    ...
    def register_middleware(self):
        ...
        for (k, v) in self.__dict__.items():
            if k.split("_")[-1] == "middleware":
                self.app.add_middleware(BaseHTTPMiddleware, dispatch=v)
...
@dataclass
class PostgresStacApi(StacApi):
    """postgres backed stac api"""
    ...
    async def db_connection_middleware(self, request: Request, call_next):
